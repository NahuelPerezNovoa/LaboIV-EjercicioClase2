...
export class RegistrarComponent {
  ...
  registrar(): void {
    if(this.mail != "" && this.contrasenia != ""){
      ...
      const strUsuarios = localStorage.getItem("clase2Usuarios");
  ...
  guardarUsuarios():void {
    ...
    localStorage.setItem("clase2Usuarios", usuariosString);
  }

  guardarUsuarioLogueado(usuario: Usuario):void {
    const usuarioString = JSON.stringify(usuario);
    localStorage.setItem("clase2UsuarioLogueado", usuarioString);
  }
